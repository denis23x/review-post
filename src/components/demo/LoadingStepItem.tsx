...
export function LoadingStepItem({ state, label }: LoadingStepItemProps) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      {state === 'active' && <Loader2 size={18} className="animate-spin text-[#4A9FD8] shrink-0" />}
      {state === 'pending' && <Circle size={18} className="text-[#E5E7EB] shrink-0" />}
      <span className={state === 'active' || state === 'done' ? 'text-sm text-[#1a1a1a]' : 'text-sm text-[#888888]'}>
        {label}
      </span>
    </div>
  )
